...
__all__ = ["MaskFormerIncrementalInstanceDatasetMapper"]


class MaskFormerIncrementalInstanceDatasetMapper:
    """
    ...
    def __init__(
        ...
        *,
        ...
        dataset, _, task, step = dataset_name.split('_')
        self.dataset = dataset
        self.task = task
        self.step = step
        self.background_label = 255
        if dataset == 'ade20k':
            self.training_ids = tasks_ade_ins[task][int(step)]
        elif dataset == 'coco':
            self.training_ids = tasks_coco_ins[task][int(step)]
        else:
    ...
    def __call__(self, dataset_dict):
        ...
        classes = torch.tensor(classes, dtype=torch.int64)+1 # [0,1,...] -> [1,2,...], 0 as background
        ...
        dataset_dict["ignore"] = torch.zeros(image.shape[1:])
